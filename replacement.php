 ...
 */
function icit_srdb_define_find( $filename = 'local-config.php' ) {
...
if ( $step == 0 || $step == 1 )
	$step = file_exists( dirname( __FILE__ ) . '/local-config.php' ) ? 1 : 2;

// Scan wp-config for the defines. We can't just include it as it will try and load the whole of wordpress.
if ( $loadwp && file_exists( dirname( __FILE__ ) . '/local-config.php' ) )
	list( $host, $data, $user, $pass, $char ) = icit_srdb_define_find( 'local-config.php' );
